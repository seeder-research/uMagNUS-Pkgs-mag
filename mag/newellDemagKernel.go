...
import (
	...
	oommf "github.com/seeder-research/uMagNUS-Pkgs-oommf/oommf"
...
func NewellDemagKernel(inputSize, pbc [3]int, cellsize [3]float64, asymptotic_radius, zero_self_demag int, cacheDir string) (kernel [3][3]*data.Slice) {
	...
	// Try to load kernel
	baseDirNamePrefix := fmt.Sprint("uMagNUS")
	if data.SIZEOF_DATATYPE > 4 {
		baseDirNamePrefix = fmt.Sprint(baseDirNamePrefix, "64kernel_")
	} else {
		baseDirNamePrefix = fmt.Sprint(baseDirNamePrefix, "kernel_")
	}
	basename := fmt.Sprint(cacheDir, "/", baseDirNamePrefix, inputSize, "_", pbc, "_", cellsize, "_", zero_self_demag, "_")
	var errLoad error
	for i := 0; i < 3; i++ {
		for j := i; j < 3; j++ {
...
func CalcNewellDemagKernel(inputSize, pbc [3]int, cellsize [3]float64, asymptotic_radius, zero_self_demag int) (kernel [3][3]*data.Slice) {
					...
					for x := r1[X]; x <= r2[X]; x++ {
						...
						if scaled_arad >= 0.0 {
							...
							if R2[X] > testVal {
								...
								array[X][X][zw][yw][xw] += data.DataType(-1.0 * ANxx.NxxAsymptotic(&ptdata))
								array[X][Y][zw][yw][xw] += data.DataType(-1.0 * ANxy.NxyAsymptotic(&ptdata))
								array[X][Z][zw][yw][xw] += data.DataType(-1.0 * ANxz.NxzAsymptotic(R[X], R[Y], R[Z]))
								array[Y][Y][zw][yw][xw] += data.DataType(-1.0 * ANyy.NyyAsymptotic(R[X], R[Y], R[Z]))
								array[Y][Z][zw][yw][xw] += data.DataType(-1.0 * ANyz.NyzAsymptotic(R[X], R[Y], R[Z]))
								array[Z][Z][zw][yw][xw] += data.DataType(-1.0 * ANzz.NzzAsymptotic(R[X], R[Y], R[Z]))
							} else {
								array[X][X][zw][yw][xw] += data.DataType(CalculateSDA00(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nxx
								array[X][Y][zw][yw][xw] += data.DataType(CalculateSDA01(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nxy
								array[X][Z][zw][yw][xw] += data.DataType(CalculateSDA02(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nxz
								array[Y][Y][zw][yw][xw] += data.DataType(CalculateSDA11(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nyy
								array[Y][Z][zw][yw][xw] += data.DataType(CalculateSDA12(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nyz
								array[Z][Z][zw][yw][xw] += data.DataType(CalculateSDA22(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nzz
							}
						} else {
							array[X][X][zw][yw][xw] += data.DataType(CalculateSDA00(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nxx
							array[X][Y][zw][yw][xw] += data.DataType(CalculateSDA01(R[X], R[Y], R[Z], dx, dy, dz) * scale) // Calculate Nxy
	...
	array[X][X][0][0][0] = -1.0 * data.DataType(SelfDemagNx(dx, dy, dz))
	array[Y][Y][0][0][0] = -1.0 * data.DataType(SelfDemagNy(dx, dy, dz))
	array[Z][Z][0][0][0] = -1.0 * data.DataType(SelfDemagNz(dx, dy, dz))

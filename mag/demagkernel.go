...
import (
	...
	timer "github.com/seeder-research/uMagNUS-Pkgs-timer/timer"
...
func DemagKernel(inputSize, pbc [3]int, cellsize [3]float64, accuracy float64, cacheDir string) (kernel [3][3]*data.Slice) {
	...
	basename := fmt.Sprint(cacheDir, "/", baseDirNamePrefix, inputSize, "_", pbc, "_", cellsize, "_", accuracy, "_")
	var errLoad error
	for i := 0; i < 3; i++ {
		for j := i; j < 3; j++ {
				...
				for y := r1[Y]; y <= r2[Y]; y++ {
					...
					for x := r1[X]; x <= r2[X]; x++ {
						...
						for d := s; d < 3; d++ { // destination index Ksdxyz
							array[s][d][zw][yw][xw] += data.DataType(B[d]) // += needed in case of PBC
						}
					}
				}
			}

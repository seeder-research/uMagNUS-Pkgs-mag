...
func MFMKernel(mesh *data.Mesh, lift, tipsize float64, cacheDir string) (kernel [3]*data.Slice) {
	...
	if data.SIZEOF_DATATYPE > 4 {
		baseDirNamePrefix = fmt.Sprint(baseDirNamePrefix, "64MFMkernel_")
	} else {
		baseDirNamePrefix = fmt.Sprint(baseDirNamePrefix, "MFMkernel_")
	}
	basename := fmt.Sprint(cacheDir, "/", baseDirNamePrefix, mesh.Size(), "_", mesh.PBC(), "_", mesh.CellSize(), "_", lift, "_", tipsize, "_")
	var errLoad error
	for i := 0; i < 3; i++ {
	...
	for i := 0; i < 3; i++ {
		...
		info := data.Meta{Time: float64(0.0), Name: compName, Unit: "1", CellSize: mesh.CellSize(), MeshUnit: "m"}
...
func LoadMFMKernel(fname string) (kernel *data.Slice, err error) {
	kernel, _, err = oommf.ReadFile(fname)
	return
}

func SaveMFMKernel(fname string, kernel *data.Slice) error {
	...
	defer out.Flush()
	if data.SIZEOF_DATATYPE > 4 {
		oommf.WriteOVF2(out, kernel, data.Meta{}, "binary 8")
	} else {
...
func CalcMFMKernel(mesh *data.Mesh, lift, tipsize float64) (kernel [3]*data.Slice) {
		...
		for iy := r1[Y]; iy <= r2[Y]; iy++ {
			...
			for ix := r1[X]; ix <= r2[X]; ix++ {
				...
				for s := 0; s < 3; s++ { // source index Ksxyz
					...
					K[s][zw][yw][xw] += data.DataType(dFdz_tip) // += needed in case of PBC
				}
			}
		}
	}
